...
def test_capi1_get_parameters():
    tests_dir = os.path.join(os.path.dirname(__file__), __name__)
...
def test_get_tool_options():
    ...
    assert {"iverilog_options": ["-DSIM"]} == core.get_tool_options(
        {"is_toplevel": True, "tool": "icarus"}
    )
    assert {} == core.get_tool_options({"is_toplevel": True, "tool": "modelsim"})
    assert {"fuse_options": ["some", "isim", "options"]} == core.get_tool_options(
        {"is_toplevel": True, "tool": "isim"}
    )
    expected = {
        "xelab_options": [
            "--timescale 1ps/1ps",
    ...
    assert {"libs": ["-lelf"]} == core.get_tool_options(
        {"is_toplevel": False, "tool": "verilator"}
    )
...
def test_icestorm():
    ...
    compare_fileset(
        core.file_sets[0], "rtl_files", ["c3demo.v", "ledpanel.v", "picorv32.v"]
    )
    compare_fileset(
        core.file_sets[1],
        "tb_files",
        ["firmware.hex", "$YOSYS_DAT_DIR/ice40/cells_sim.v", "testbench.v"],
    )
