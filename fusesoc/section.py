        ...
        for src_file in self.src_files:
            ...
            l = Launcher('gcc',
                     args + [os.path.join(src_root, core, src_file)],
                         cwd=sim_root,
                         stderr = open(os.path.join(sim_root, os.path.basename(src_file) +'.err.log'),'w'),
                         stdout = open(os.path.join(sim_root, os.path.basename(src_file) +'.out.log'),'w'))
            if Config().verbose:
    ...
    def build_CPP(self, core, sim_root, src_root):
        ...
        for src_file in self.src_files:
            ...
            l = Launcher('g++', args + [os.path.join(src_root, core, src_file)],
                         cwd=sim_root,
                         stderr = open(os.path.join(sim_root, os.path.basename(src_file) +'.log'),'w'))
            if Config().verbose:
                pr_info("  C++ compilation working dir: " + sim_root)
