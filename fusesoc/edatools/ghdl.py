...
class Ghdl(EdaTool):
    ...
    def configure_main(self):
        ...
        if 'analyze_options' in self.tool_options:
            stdarg += self.tool_options['analyze_options']

        run_options = []
        if 'run_options' in self.tool_options:
            run_options = self.tool_options['run_options']

        makefile = open(os.path.join(self.work_root, 'Makefile'), 'w')
        makefile.write("""#Auto generated by FuseSoC
...
run:
	ghdl --elab-run $(TOPLEVEL) $(RUN_OPTIONS) $(EXTRA_OPTIONS)
analyze:
""".format(toplevel=self.toplevel,
